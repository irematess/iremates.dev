<script></script>
<template>
  <div class="mx-auto p-4 px-8 rounded-lg italic text-4xl font-bold text-primary">PROJECTS</div>
  <div class="w-1/2 mx-auto flex flex-col gap-8">
    <div class="flex bg-white box p-8 rounded gap-2 items-center">
      <figure>
        <img src="../assets/fast-finger-app.png" class="w-[35rem]" />
      </figure>
      <div class="text-md flex flex-col justify-between">
        <span class="text-xl italic text-primary">10 Fast Finger App</span>
        <div class="">
          An application that you can test your typing speed that I developed using event listeners.

          <span class="bg-green-200 rounded-3xl px-2 mr-2"
            ><span class="mr-1 text-green-500">•</span>Vue
            <i class="fa-brands fa-vuejs text-green-500"></i></span
          ><span class="bg-sky-200 rounded-3xl px-2"
            ><span class="mr-1 text-sky-500">•</span>Tailwind</span
          >
        </div>

        <div class="flex gap-4 py-2">
          <a
            href="https://10fast-finger-app.vercel.app/"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Demo</a
          >
          <a
            href="https://github.com/irematess/10fastFingerApp"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Code</a
          >
        </div>
      </div>
    </div>
    <div class="flex bg-white box p-8 rounded gap-2 items-center">
      <figure>
        <img src="../assets/e-commerce-app.png" class="w-[55rem]" />
      </figure>
      <div class="text-md flex flex-col">
        <span class="text-xl italic text-primary">E-Commerce App</span>
        I developed the interface of the project using tailwind and Vue.JS. I processed the data
        using Axios and the json-server package I did the routing with the Vue Router packag
        <div class="flex gap-4 py-2">
          <a
            href="https://github.com/irematess/e-commerce-app"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Code</a
          >
        </div>
      </div>
    </div>
    <div class="flex bg-white box p-8 rounded gap-2 items-center">
      <figure>
        <img src="../assets/user-list-app.png" />
      </figure>
      <div class="text-md flex flex-col">
        <span class="text-xl italic text-primary">User List App</span>
        I developed the interface of the project using tailwind and Vue.JS. Using Rest Api, i
        received data from external sources and showed this data to the user. I learned how to use
        the Axios package and Vue Router. I took an avatar picture according to the username using
        Avatar Api.

        <div class="flex gap-4 py-2">
          <a
            href="https://user-list-app-eta.vercel.app/"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Demo</a
          >
          <a
            href="https://github.com/irematess/userListApp"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Code</a
          >
        </div>
      </div>
    </div>
    <div class="flex bg-white box p-8 rounded gap-2 items-center">
      <figure>
        <img src="../assets/movie-app.png" />
      </figure>
      <div class="text-md flex flex-col justify-between">
        <span class="text-xl italic text-primary">Movie App</span>
        <div class="">
          I developed the interface of the project using Vue.JS Using Rest Api, i received data from
          external sources and showed this data to the user. I processed the data using Axios. I did
          the routing with the Vue Router package
          <span class="bg-green-200 rounded-3xl px-2 mr-2"
            ><span class="mr-1 text-green-500">•</span>Vue
            <i class="fa-brands fa-vuejs text-green-500"></i></span
          ><span class="bg-sky-200 rounded-3xl px-2"
            ><span class="mr-1 text-sky-500">•</span>Tailwind</span
          >
        </div>

        <div class="flex gap-4 py-2">
          <a
            href="https://movie-app-xhhv.vercel.app/"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Demo</a
          >
          <a
            href="https://github.com/irematess/movie-app/tree/main/movie-app"
            class="p-2 px-4 border-2 border-primary text-primary rounded-3xl"
            >Code</a
          >
        </div>
      </div>
    </div>
  </div>
</template>
